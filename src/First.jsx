...
function BuyElectricity() {
  ...
  useEffect(() => {
    // Load recents from localStorage when the component mounts
  ...
  const handlePackageChange = (event) => {
    ...
    setIsPackageModalOpen(false); // Close the modal after selection
  };

  const handleMeterNumberChange = (event) => {
    setMeterNumber(event.target.value);
    setSelectedMeter(''); // Reset selected meter when typing manually
  };
  ...
  const handleProviderClick = (provider) => {
    ...
    setIsProviderModalOpen(false); // Close the modal after selection
  };
  ...
  const handleAddToRecents = () => {
    // Check if the meter number is already in the recents list
    if (!recents.some((meter) => meter.number === meterNumber)) {
      // Create a new entry with a unique ID and the meter number
      const newRecents = [...recents, { id: recents.length + 1, number: meterNumber }];
      
      // Update the state with the new recents list
      setRecents(newRecents);
  
      // Save the updated recents list to local storage
    ...
    navigate('/select-provider'); // Replace with your actual next page route
  };
  ...
  return (
    <div className="relative bg-gray-100 min-h-screen">
      <header>
        <h1 className="text-2xl font-bold mb-4 text-center pt-8">Buy Electricity</h1>
      </header>
      <hr className="text-gray-800 mb-6 border border-gray-200" />

      <form onSubmit={handleSubmit} className="bg-white rounded-lg px-4 py-6 mb-4">
        <div className="mb-4">
          <h2 className="font-bold text-lg mb-4 font-sans">Enter your correct details</h2>
          <button
            type="button"
        ...
        <div className="flex items-center justify-center">
          <button
            type="submit"
            className={`bg-amber-200 hover:bg-amber-400 text-white font-bold py-4 px-4 rounded-lg focus:outline-none focus:shadow-outline w-full text-xl `}
      ...
      <div className="bg-white shadow-md rounded-t-3xl p-4 ml-5 mr-5 h-96">
        ...
        <ul className="list-none">
          {recents.length === 0 ? (
            <li className="text-gray-300 flex justify-center items-center m-auto mt-28 text-2xl font-bold">Add To Recents</li>
          ) : (
            recents.map((meter) => (
              <li
                key={meter.id}
      ...
      {isConfirmationModalOpen && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-lg p-8 rounded-lg shadow-lg relative">
            <h2 className="text-2xl font-semibold mb-4 text-gray-700">Add to Beneficiary?</h2>
            <p className="mb-6 text-gray-600">
              Do you want to add meter number {meterNumber} to your beneficiary list?
            </p>
